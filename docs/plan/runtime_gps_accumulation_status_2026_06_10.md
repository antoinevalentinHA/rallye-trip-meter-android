...
> **Mise à jour 2026-06-11 (chântier UI ergonomie rallye — clôture)** : écran principal rendu plus utilisable au doigt — carte PARTIEL **cliquable par tap simple** pour le reset partiel (`6f9eeb0`), **bouton RAZ PARTIEL retiré** de la rangée (`1a4f2c8`), boutons **± agrandis** (`fcd8588`), **vide vertical central réduit** (`SpaceBetween` externe remplacé, `2306a76`), **précision GPS affichée une seule fois** (`1e1d124`). **Validé Gradle local + APK** ; **aucun** changement runtime / moteur GPS / service / filtre / calibration ; aucun changement métier hors interaction UI. **Réserves** : test terrain rallye à refaire, calibration non validée, multi-appareils non validé, UI finale perfectible. Détail : [`rally_ui_ergonomics_2026_06_11.md`](rally_ui_ergonomics_2026_06_11.md).

> **Mise à jour 2026-06-11 (jalon — version utilisable rallye simple)** : état de fin de journée acté comme **version proche utilisable en conditions simples de rallye / roadbook** — moteur d'accumulation validé device (un appareil), service foreground fonctionnel, écran éteint / pause-reprise / terminer validés device, permissions Android corrigées, **précision GPS affichée** (`7b15bc9`), **Terminer protégé par confirmation** (`2b61c5b`), runtime/UI découplés. **Réserves** : un seul appareil, **calibration non validée**, précision **non certifiée**, robustesse OEM non exhaustive, UI non finalisée, **aucun tag Git** créé. Jalon : [`usable_rally_milestone_2026_06_11.md`](usable_rally_milestone_2026_06_11.md).
